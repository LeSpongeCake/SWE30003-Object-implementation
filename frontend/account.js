(() => {
  ...
  function renderAccountLikePage(mode = "account") {
    ...
    if (session.kind === "admin") {
      profileCard.innerHTML = `
        <div class="panel-block">
          ...
          <h2>Account only for user accounts</h2>
          <p>Please use the admin console instead.</p>
        </div>
      `;
      activityCard.innerHTML = `
        <div class="panel-block">
          ...
          <div class="action-stack">
            <a class="button-link secondary" href="admin.html">Go to Admin console</a>
          </div>
        </div>
      `;
      if (orderList) {
        orderList.innerHTML = `<article class="empty-state"><h2>Functionality not for admin</h2><p>Admins do not have purchase history.</p></article>`;
      }
    } else {
      profileCard.innerHTML = `
        <div class="panel-block">
        ...
        <dl class="summary-grid compact">
          <div><dt>Session</dt><dd>${escapeHtml(String(getSessionIdentifier()).slice(-8))}</dd></div>
    ...
    if (signOutButton) {
      if (mode === "guest" && session.kind !== "guest") {
        signOutButton.textContent = "Sign out and continue as guest";
      }
      signOutButton.addEventListener("click", () => {
        createGuestSession();
        window.location.href = mode === "guest" ? "guest.html" : "index.html";
      });
    }
  }
